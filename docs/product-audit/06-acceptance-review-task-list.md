...
| 路径 | 用途 | 状态 |
| --- | --- | --- |
| `.research-network/secrets/acceptance-buyer.json` | buyer zkLogin session，敏感，不提交。 | [ ] 待重新生成或重新校验 |
| `.research-network/secrets/acceptance-agent.json` | agent zkLogin session，敏感，不提交。 | [ ] 待重新生成或重新校验 |
