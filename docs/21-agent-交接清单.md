...
>
> 一句话现状：**Web/Vite 前端与真实 Walrus + Seal + Sui 交易路径已经接入，最新 Seal Access 包已重新发布到 testnet 并完成 M4 author decrypt 回归；但 mainnet 仍未放行，还需要两个真实 zkLogin 账号跑带资金上限的 production acceptance。**
