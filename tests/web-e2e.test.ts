...
describe("static web E2E", () => {
  ...
  it("builds a Vercel auth shell without shadowing Walrus content pages", async () => {
    ...
    expect(indexHtml).toContain("data-chain-submissions");
    expect(indexHtml).toContain("ResearchAssetPublished");
    expect(indexHtml).toContain("/site.js?v=20260624-live-skills-v4");
    ...
    expect(shellSiteData.assets).toEqual([]);
    const searchHtml = await fs.readFile(path.join(shellDir, "search.html"), "utf8");
    expect(searchHtml).toContain("data-live-search");
    ...
    expect(delegationsHtml).not.toContain("0x4f1c");
    ...
    expect(await exists(path.join(shellDir, "paper", loopSeg, "main.pdf"))).toBe(false);
    expect(await exists(path.join(shellDir, "paper", loopSeg, "main.tex"))).toBe(false);
