...
function installDom(): void {
  ...
  Object.defineProperty(dom.window.URL, "createObjectURL", {
    value: vi.fn((blob: Blob) => {
      capturedBlob = blob;
      return "blob:debug-acceptance-session";
    }),
    configurable: true
  });
...
async function renderDebugPage(): Promise<void> {
  const pageModulePath = "../web/src/pages/DebugPage.tsx";
  const { DebugPage } = await import(pageModulePath);
...
describe("DebugPage acceptance tools", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    capturedBlob = null;
  ...
  it("exports the current same-tab zkLogin session from the debug route", async () => {
    seedSignedInSession();
    await renderDebugPage();

    await clickByTestId("debug-export-acceptance-agent");

    expect(clickedDownload).toEqual({
      download: "acceptance-agent.json",
      href: "blob:debug-acceptance-session"
    });
  ...
  it("fails closed when the same-tab ephemeral key is unavailable", async () => {
    ...
    await clickByTestId("debug-export-acceptance-buyer");

    expect(clickedDownload).toBeNull();
    expect(capturedBlob).toBeNull();
    expect(byTestId("debug-acceptance-session-export-status").textContent).toContain("rn_zk_eph.secret");
  });
});
