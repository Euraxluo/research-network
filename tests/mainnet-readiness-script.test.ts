...
describe("mainnet readiness script", () => {
  ...
  it("fails mainnet-final when the testnet execute receipt transactions are not live on chain", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rn-readiness-"));
    let stdout = "";
    let stderr = "";
    let testnetExecuteReceipt: ProductionAcceptanceReceipt | null = null;
    let mainnetExecuteReceipt: ProductionAcceptanceReceipt | null = null;
    ...
    const server = http.createServer(async (request, response) => {
      ...
      if (body.method === "sui_multiGetTransactionBlocks") {
        const digests = body.params[0] as string[];
        const receipt = transactionQueryCount === 0 ? testnetExecuteReceipt : mainnetExecuteReceipt;
        transactionQueryCount += 1;
        response.end(JSON.stringify({
          jsonrpc: "2.0",
          id: body.id,
          result: digests.map((digest, index) =>
            receipt === testnetExecuteReceipt && index === 0
              ? null
              : chainTransactionBlock(digest, receipt)
          )
        }));
        return;
      }
    ...
    try {
      ...
      const mainnetExecutePath = path.join(dir, "mainnet-execute.json");
      await fs.writeFile(testnetPreflightPath, JSON.stringify(makePreflightReceipt(), null, 2), "utf8");
      testnetExecuteReceipt = makeExecuteReceipt();
      await fs.writeFile(testnetExecutePath, JSON.stringify(testnetExecuteReceipt, null, 2), "utf8");
      const mainnetReceiptConfig = {
        ...mainnetConfig(),
        suiRpcUrl: rpcUrl
      };
      await fs.writeFile(mainnetPreflightPath, JSON.stringify({
        ...makePreflightReceipt("mainnet"),
        config: mainnetReceiptConfig
      }, null, 2), "utf8");
      mainnetExecuteReceipt = makeExecuteReceipt("mainnet", {
        config: mainnetReceiptConfig
      });
      ...
      expect(report.ready).toBe(false);
      expect(report.checks.some((check) =>
        check.name.startsWith("chain.testnet.transaction.") &&
        check.status === "failed" &&
        /not found/.test(check.message)
      )).toBe(true);
    } finally {
      server.close();
      await fs.rm(dir, { recursive: true, force: true });
      ...
      if (body.method === "sui_multiGetTransactionBlocks") {
        ...
        response.end(JSON.stringify({
          ...
          result: digests.map((digest) => ({
            digest,
            timestampMs: String(Date.parse("2026-06-17T00:00:30.000Z")),
  ...
  it("fails mainnet-final when a live transaction sender does not match the receipt role", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rn-readiness-"));
    let stdout = "";
    let stderr = "";
    let mainnetExecuteReceipt: ProductionAcceptanceReceipt | null = null;
    ...
    const server = http.createServer(async (request, response) => {
      ...
      if (body.method === "sui_multiGetTransactionBlocks") {
        ...
        response.end(JSON.stringify({
          jsonrpc: "2.0",
          id: body.id,
          result: digests.map((digest) => chainTransactionBlock(
            digest,
            mainnetExecuteReceipt,
            receiptTransactionTimestampMs(mainnetExecuteReceipt),
            digest === mismatchedDigest ? "0x" + "99".repeat(32) : undefined
          ))
        }));
        return;
      }
    ...
    try {
      ...
      mismatchedDigest = mainnetExecuteReceipt.steps.find((step) => step.name === "buyer.buy_platform_membership")?.digest ?? "";
      await fs.writeFile(mainnetExecutePath, JSON.stringify(mainnetExecuteReceipt, null, 2), "utf8");
      ...
      expect(report.checks.some((check) =>
        check.name.startsWith("chain.mainnet.transaction.") &&
        check.status === "failed" &&
        /sender did not match/.test(check.message)
      )).toBe(true);
    } finally {
      server.close();
      await fs.rm(dir, { recursive: true, force: true });
  ...
  it("fails mainnet-final when a live transaction is outside the execute receipt window", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rn-readiness-"));
    let stdout = "";
    let stderr = "";
    let mainnetExecuteReceipt: ProductionAcceptanceReceipt | null = null;
      ...
      if (body.method === "sui_multiGetTransactionBlocks") {
        ...
        response.end(JSON.stringify({
          ...
          result: digests.map((digest) => chainTransactionBlock(
            digest,
            mainnetExecuteReceipt,
            Date.parse("2026-06-16T23:59:59.000Z")
          ))
        }));
        return;
      }
    ...
    try {
      ...
      expect(report.checks.some((check) =>
        check.name.startsWith("chain.mainnet.transaction.") &&
        check.status === "failed" &&
        /outside the receipt window/.test(check.message)
      )).toBe(true);
    } finally {
      server.close();
      await fs.rm(dir, { recursive: true, force: true });
...
function chainTransactionBlock(
  digest: string,
  receipt: ProductionAcceptanceReceipt | null,
  timestampMs = receiptTransactionTimestampMs(receipt),
  ...
  return {
    ...
    transaction: {
      data: {
        sender: senderOverride ?? receiptSignerForDigest(receipt, digest)
      }
    },
    effects: { status: { status: "success" } },
...
function receiptTransactionTimestampMs(receipt: ProductionAcceptanceReceipt | null): number {
  const started = Date.parse(receipt?.startedAt ?? "");
  const finished = Date.parse(receipt?.finishedAt ?? "");
  if (Number.isFinite(started) && Number.isFinite(finished) && finished >= started) {
    return started + Math.floor((finished - started) / 2);
  }
  return Date.parse("2026-06-17T00:00:30.000Z");
}
...
function receiptSignerForDigest(receipt: ProductionAcceptanceReceipt | null, digest: string): string | undefined {
  ...
  for (const step of receipt.steps) {
    if (step.digest === digest && typeof step.meta?.signerAddress === "string") {
      return step.meta.signerAddress;
    }
    if (step.meta?.fundDigest === digest && typeof step.meta?.fundSignerAddress === "string") {
      return step.meta.fundSignerAddress;
    }
  }
  return undefined;
}
