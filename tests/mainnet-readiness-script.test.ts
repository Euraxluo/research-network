...
describe("mainnet readiness script", () => {
  ...
  it("fails when receipt provenance was generated by a different code checkout", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rn-readiness-"));
    let stdout = "";
    let stderr = "";
    try {
      const preflightPath = path.join(dir, "testnet-preflight.json");
      const executePath = path.join(dir, "testnet-execute.json");
      const staleExecuteReceipt = makeExecuteReceipt();
      staleExecuteReceipt.provenance = {
        ...receiptProvenance(),
        gitCommit: "b".repeat(40)
      };
      await fs.writeFile(preflightPath, JSON.stringify(makePreflightReceipt(), null, 2), "utf8");
      await fs.writeFile(executePath, JSON.stringify(staleExecuteReceipt, null, 2), "utf8");
      ...
      expect(report.ready).toBe(false);
      expect(report.checks.some((check) =>
        check.name === "receipt.testnet-execute.provenance.current_commit" &&
        check.status === "failed" &&
        /current code checkout/.test(check.message)
      )).toBe(true);
    } finally {
      await fs.rm(dir, { recursive: true, force: true });
    }
