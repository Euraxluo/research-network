...
The user-facing account page contains production account content only. Acceptance session collection
...
Run preflight/execute from a committed, clean Git worktree. Each receipt records
`provenance.gitCommit`, `provenance.gitTreeState`, and package metadata; readiness rejects receipts
that are missing provenance, were generated from a dirty tree, or do not match the current checkout.
The testnet receipts must also match the expected current testnet version from `DEFAULT_M3_CONFIG`
exactly: package id, shared object ids, Sui/Walrus/Seal endpoints, thresholds, and economic
